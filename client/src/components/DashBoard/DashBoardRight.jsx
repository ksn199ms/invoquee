...
const DashboardRight = () => {
  return (
    <div className="pt-15 pl-3 sm:p-6 bg-gray-50 min-h-screen">
      <h2 className="text-xl  sm:text-2xl font-semibold mb-6">Dashboard</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mb-6">
        {stats.map((stat, index) => (
          <div
            key={index}
            className="bg-white p-4 rounded-xl shadow-sm flex flex-col gap-2 hover:shadow-md transition"
          >
            <div className="flex items-center gap-3">
              <div className={`p-2 sm:p-3 rounded-full ${stat.bg} text-lg sm:text-xl`}>
                {stat.icon}
              </div>
              <div>
                <p className="text-xs sm:text-sm text-gray-500">{stat.title}</p>
                <h4 className="text-base sm:text-lg font-bold">{stat.value}</h4>
              </div>
            </div>
            <p
              className={`text-xs sm:text-sm font-medium ${
                stat.direction === "down" ? "text-red-500" : "text-green-500"
              }`}
            >
              {stat.direction === "down" ? "▼" : "▲"} {stat.change} {stat.note}
            </p>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
          <h4 className="text-sm sm:text-md font-semibold">Sales Details</h4>
          <select className="border text-xs sm:text-sm px-2 py-1 rounded-md text-gray-600">
            <option>October</option>
          </select>
        </div>
